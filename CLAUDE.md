...
- **Running tests / docs needs the rocker container + a GDAL bind.** The user's R package library (`~/R/rocker-rstudio/4.5`, R 4.5) only loads inside `/cluster/scratch/$USER/rocker_rstudio_4.5.sif`; the Euler module R (4.4.1) can't load it (GLIBC). `terra`/`sf` additionally need `libgdal.so.33`/`libproj.so.25`/`libgeos_c.so.1`/`libudunits2.so.0`, absent in the bare container — inject a soname-matching set via `LD_LIBRARY_PATH` (GDAL+PROJ from the paraview manual install, GEOS+udunits from the module stack). `tools/check_nn_generalization.sh` is the ready-made SLURM job that does all of this and runs `devtools::test()` + `roxygenise()`; it last reported `PASS=271 FAIL=0`. (This works because the tests build rasters in-memory via `terra::rast(df, type="xyz")` and never touch GDAL drivers.) For terra/sf-free logic, `tools/nn_core_harness.R` validates the FNN-only NN sampling core (dimension-generality, the `√(d/2)` threshold, density flattening, exclusion direction) with no GDAL needed. See the `r-execution-environment` memory for the exact lib paths.
