...
env.data.raster.all.dim <- geodata::worldclim_global(var='bio',
                                                     res=10,
                                                     path=data.dir)  %>%
  terra::crop(terra::ext(-12, 25, 36, 60))

selected.layers <- sample(terra::nlyr(env.data.raster.all.dim),
...
```
The sampled points were rearranged so that the data can be easier compared. 
...
# One core has to be used to render the vignette. This number should be higher for actual use
