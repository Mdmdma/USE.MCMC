...
# The first figure draws a country outline from rnaturalearth + rnaturalearthdata
# (data-only optional packages). If they are not installed we simply skip that
# illustrative map instead of failing the whole vignette.
have_naturalearth <- requireNamespace("rnaturalearth", quietly = TRUE) &&
...
```{r, geographical-space-example, fig.height=5, fig.width=7, echo=FALSE, eval=have_naturalearth}
...
```{r, geographical-space-points, echo=FALSE}
# City + test-point coordinates and the city->test-point distance helper. These are
# reused in environmental and PCA space further down, so they are defined here
# unconditionally; only the country-outline maps need rnaturalearth/rnaturalearthdata.
...
city.points.sf <- sf::st_as_sf(city.points, coords = c("lon", "lat"), crs = 4326)
test.point <- data.frame(name = "Test point", lon = 12.6, lat = 43.7)
test.point.sf <- sf::st_as_sf(test.point, coords = c("lon", "lat"), crs = 4326)
create.lines.with.distances <- function(cities.sf, test.point.sf) {
  lines.list <- list()
  distances <- numeric(nrow(cities.sf))
  for (i in 1:nrow(cities.sf)) {
    line.coords <- rbind(st_coordinates(cities.sf[i, ]),
                         st_coordinates(test.point.sf))
    lines.list[[i]] <- st_linestring(line.coords)
    distances[i] <- as.numeric(st_distance(cities.sf[i, ], test.point.sf)) / 1000
  }
  st_sf(city = cities.sf$name, distance.km = round(distances, 1),
        geometry = st_sfc(lines.list, crs = st_crs(cities.sf)))
}
```

```{r, geographical-space-example-part-2, fig.height=5, fig.width=7, echo=FALSE, eval=have_naturalearth}
p.real.space <- ggplot() +
...
# Create lines with distances (helper defined in the chunk above)
...
In this case Venice is closest to the test point. If we want to get a real point from our test point using geographical distance as our metric we would therefore choose Venice. 
...
Looking at the first two principal components we see that the test point is climate wise closest to Turin, while still being far. In addition we see that the distance to Naples, Milan and Venice is not much higher. The distance gives us an indication on how similar the best match is. In this case it would probably be sound to say that no good match was found. As our environmental space only contained 7 points this is no surprise. Luckily for us there are more than seven measurement stations in Europe and satellites can provide high resolution data for areas without measurement devices on the ground. At a resolution of 2.5 arc minutes there are over 18'000 data points in western Europe. 
...
p.env.space.pc <- ggplot(env.with.pc.sf) + 
  geom_sf(size=0.1) +
  theme_minimal() + 
  labs(title = "The environment in principal components") + 
  xlab("PC1") + ylab("PC2")

print(p.env.space.pc)
```
Now let's explore how the nearest neighbor mapping can help us to uniformly sample points out of this environmental space.
...
The function [`paSamplingNn()`][USE.MCMC::paSamplingNn] performs this sampling given the environmental space. Let's look under its hood to understand how it works. 
...
```{r repeat-grid}
# Repeat every grid point num.samples.per.cell times (rows 1..N, then 1..N again,
# ...), so we can later draw several pseudo-absences from the same cell.
grid.repeated <- grid[rep(seq_len(nrow(grid)), num.samples.per.cell), ]
```

So far this does not work, as these repeated points would all map to the same nearest neighbor in the environment. In addition to that the grid adds a bias to the sampled points.

To eliminate this issue we add uniform noise in the range of the grid spacing to the points. Thereby we lose the grid bias, and sample from a random location in each grid cell, while on larger scales still conserving uniform sampling. In addition to that we can now sample multiple points from the same grid cell, as the noise is different for each point. 
...
Now we have points distributed nearly uniformly in the environment. The issue remaining is that we oversampled the border regions, as even cells that did not contain any points were remapped to the closest point, even if that point was in a different grid cell. We now have different options on how to set the distance threshold at which we denote points to have originated too far away. 

One option is to use the cell size as the limiting factor. By setting the distance threshold to be half the cell size, we can eliminate points that were sampled in empty cells. This approach depends on having chosen the cell size well. We can look at the limits.
...
Luckily we have additional information. The original dataset itself gives us some indication what kind of distances are normal. By searching the closest few, but at least more than the number of points per grid cell, neighbors, we can compute a histogram of the distance between points in the original data
...
print(p.neighbor.distance.histogramm)
```
We see that the distance histogram is dominated by the low distances originating in regions with high densities. If we look at the top end on the other hand, we get information on the spacing of areas with a low point density. We probably do not want to take the maximal distance as our threshold, as the dataset could contain outliers. We can sort the vector containing the distances and take a value with some safety threshold from the extreme values. 

```{r find-advanced-threshold}
sorted.nearest.neighbor.distances <- sort(nearest.neighbors.distance, 
                                          decreasing=TRUE)
# Use the 2nd-largest neighbour distance (skip the single largest, which may be an
# outlier), halved, as the cut-off: remapped points further than this from any real
# environment cell are dropped.
...
p.removed.points <- ggplot(removed.points) + 
  geom_point(aes(x=PC1, y=PC2)) +
  labs(title = "Duplicated points that were removed")
...
This threshold method can also be used to estimate an upper bound to the useful grid resolution. A function that provides this functionality is given by [`maxResNn()`][USE.MCMC::maxResNn]. 

In difference to the original [`paSampling()`][USE.MCMC::paSampling] method this approach should still converge to uniform sampling for too low grid resolutions. The issue here is that it can not be guaranteed that the distribution is flat for small sample sizes. The deviations from uniform sampling are not expected to be at the same position as the high point density areas of the underlying distribution
...
# Exclude any candidate pseudo-absence that falls inside the species' realised
# niche. The strategy:
#   1. estimate how concentrated the presences are in PC space (a kernel density
#      estimate, ks::kde),
#   2. treat the densest `thres` fraction of presences as the niche core,
#   3. wrap that core in a convex hull, and
#   4. drop every pseudo-absence that lands inside the hull.
occ.vec <- virtual.presence.points
thres = 0.7   # keep the densest 70% of presences as the niche core
...
PC12occ <- terra::extract(id_rast,  occ.vec, cells = FALSE,
                          df = FALSE, ID=FALSE)[,1]
presence_absence_pc <- na.omit(data.frame(dt, PA= ifelse(dt$myID %in% PC12occ, 1, 0)))

H <- ks::Hpi(x = presence_absence_pc[, c("PC1", "PC2")])

estimate <- data.frame(KDE = ks::kde(presence_absence_pc[presence_absence_pc$PA == 1, c("PC1", "PC2")],
                                     eval.points = presence_absence_pc[presence_absence_pc$PA ==1,
                                                          c("PC1", "PC2")],
                                     h = H)$estimate,
                       presence_absence_pc[presence_absence_pc$PA == 1, c("PC1", "PC2", "myID", "PA")])

density_cutoff <- quantile(estimate[, "KDE"], thres)
estimate$in_niche <- ifelse(estimate$KDE <= density_cutoff, "out", "in")
point.data <- merge(x = presence_absence_pc, 
                    y = estimate[estimate$PA == 1,c("myID","in_niche")],
                  by = "myID", all.x = TRUE)
point.data$in_niche <- ifelse(is.na(point.data$in_niche), "pabs",point.data$in_niche)
niche_hull <- sf::st_as_sf(subset(point.data, point.data$in_niche=="in", 
                             select=c( "PC1","PC2" )),
                      coords=c( "PC1","PC2" ))
niche_hull <- sf::st_union(niche_hull)
niche_hull <- sf::st_convex_hull(niche_hull)
...
excluded_points <- cbind(
  sf::st_drop_geometry(inside.the.region.with.presence), 
  sampled.points.presence)

pseudo_absences$type = "Pseudo-absence"
excluded_points$type = "Excluded"
...
We can see that the points that were uniformly sampled in the environmental space are not evenly spaced any more. Areas with large environmental gradients get sampled more often. 
