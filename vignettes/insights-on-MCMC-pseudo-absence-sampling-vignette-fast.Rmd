---
title: "Insights on Markov chain based pseudo-absence sampling (fast path)"
output:
  rmarkdown::html_vignette:
    self_contained: false
vignette: >
  %\VignetteIndexEntry{insights-on-MCMC-pseudo-absence-sampling-vignette-fast}
...
This is a 1-to-1 mirror of [Insights on Markov chain based pseudo-absence sampling](insights-on-MCMC-pseudo-absence-sampling-vignette.html) that exercises the same chunks against the optimized library paths (terra-based combined env+PC frame, gated verbose output, batched GMM density evaluator, partial-sort distance thresholding). The narrative is identical; only the wall-clock differs. The paper figure pipeline continues to read from the canonical vignette — chunk labels here are suffixed with `-fast` to keep that mapping untouched.

```{r timing-badge-fast, echo=FALSE, message=FALSE}
.start.time <- Sys.time()
```
...
```{r generate-sample-chain-fast, message=FALSE, results="hide"}
...
sampled.points <- USE.MCMC::mcmcSampling(dataset=starting.point,
                               ...
                               verbose = FALSE)
```
```{r animate-sample-chain-fast, eval=FALSE, echo=FALSE}
...
```{r generate-sample-gaussian-chain-fast, results="hide"}
...
```{r animate-sample-gaussian-chain-fast, echo=FALSE, eval=FALSE}
starting.point$density <- 1
...
```{r gaussian-posterior-plot-fast, echo=FALSE, fig.height=5, fig.width=7, warning=FALSE}
...
```{r sample-gaussian-large-number-of-samples-fast}
sampled.points.gaussian <- mcmcSampling(dataset=starting.point,
                                  dimensions = c("x", "y"),
                                  densityFunction = densityFunctionGaussian,
                                  proposalFunction = addHighDimGaussian(dim=2),
                                  ...
                                  verbose = FALSE)
```
```{r animate-sample-gaussian-chain-long-fast, echo=FALSE, eval=FALSE}
starting.point$density <- 1
sampled.points.gaussian <- rbind(starting.point, sampled.points.gaussian)
sampled.points.gaussian$step <- 1:nrow(sampled.points.gaussian)
p.example.chain.gaussian <- ggplot(sampled.points.gaussian) + 
...
```{r gaussian-posterior-plot-more-samples-fast, echo=FALSE, fig.height=5, fig.width=7, warning=FALSE}
p.gaussian.posterior.x.limited.samplesize <- ggplot(sampled.points.gaussian) +
  geom_density(aes(x=x)) +
  xlim(-5,5)
...
```{r plot-env-data-fast, message=FALSE, fig.height=4, fig.width=7, echo=FALSE}
...
```{r fit-environmental-model-fast, message=FALSE, fig.height=4, fig.width=7}
...
```{r set-environmental-threshold-fast}
...
```{r set-naive-density-funciton-fast}
...
```{r plot-simple-denisty-fast, echo=FALSE, fig.height=4, fig.width=7}
gridsize <- 100
grid <- expand.grid(
...
```{r plot-density-threshold-sweep-fast, echo=FALSE, fig.height=4, fig.width=7}
...
```{r sample-naive-points-fast, results="hide"}
...
```{r animate-naive-points-fast, eval=FALSE, echo=FALSE}
...
```{r generate-virtual-species-fast, results="hide"}
...
```{r fit-species-model-fast, message=FALSE, fig.height=4, fig.width=7}
...
```{r full-pseudo-absence-density-funciton-fast}
...
```{r plot-pseudo-absence-density-fast, echo=FALSE, fig.height=4, fig.width=7}
grid$density <- purrr::map_dbl(1:nrow(grid), ~ densityFunction(grid[.x, , drop = FALSE]))
...
```{r pseudo-absence-sweep-precompute-fast, echo=FALSE}
...
```{r pseudo-absence-sweep-contours-fast, echo=FALSE, fig.height=4, fig.width=7}
strong.q   <- 0.5
...
```{r pseudo-absence-sweep-ridge-fast, echo=FALSE, fig.height=3.5, fig.width=7}
pc2.fixed <- 0.5
...
```{r sample-pseudo-absences-fast, results="hide"}
...
```{r animate-pseudeo-absences-fast, eval=FALSE}
...
The figure below visualises the remapping step on a random subset of the chain: each arrow points from a sampled chain point to its nearest real environmental point, coloured by the distance travelled.

```{r plot-remapping-arrows-fast, echo=FALSE, fig.height=4, fig.width=7}
...
```{r neighbor-distance-threshold-fast}
...
```{r points-in-the-environment-fast, echo=FALSE, fig.height=4, fig.width=7, message=FALSE}
...
```{r mcmc-posterior-fast, echo=FALSE, fig.height=5, fig.width=7}
...
```{r plot-in-geo-fast, echo=FALSE, fig.height=4, fig.width=7}
...
```{r plot-pipeline-scheme-fast, echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=10}
...
```{r sample-in-higher-dim-fast, message=FALSE, results="hide"}
...
```{r plot-3d-fast, echo=FALSE, fig.height=5, fig.width=7}
p.mcmc.posterior.pc1 <- ggplot() +
...
```{r highe-dim-convergence-fast, fig.height=4, fig.width=7}
dimensions <- c("PC1", "PC2", "PC3")
...
```{r gelman-plot-fast, fig.height=5, fig.width=7}
...
```{r entropy-plot-fast, echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
...
```{r autocorreltation-plot-fast, echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
...
```{r uniform.paper-fast, message=FALSE, results='hide'}
...
```
```{r uniform.nn-fast, message=FALSE}
n.sample.points <- 300
...
```{r compare-to-uniform-models-3d-fast, echo=FALSE, fig.height=7, fig.width=7}
p.pc1 <- ggplot() +
...
```{r generate-combined-plot-fast, echo=FALSE, fig.height=9, fig.width=7}
library(cowplot)
...
```{r timing-report-fast, echo=FALSE}
.elapsed <- difftime(Sys.time(), .start.time, units = "secs")
cat(sprintf("Fast vignette wall-clock: %.1f s\n", as.numeric(.elapsed)))
```
