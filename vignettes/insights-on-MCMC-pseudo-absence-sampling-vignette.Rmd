...
Before we can understand why Markov chains are usefull, we have to understand how they work. 
...
In practical cases a Markov chain does not perform a completely random walk. The chain tries to replicate some kind of target function. In the case above this function is one everywhere. Therefore the chain explores in all directions and without limit. In this unbound case the chain does not converge to a stationary state. 

In a real usecase we want to generate samples from a target density. We can supply the MCMC algorithm with this function that represents the distribution we want to replicate. 
...
\alpha = \frac{d(p')}{d(p)}, \quad p_{n+1} = \begin{cases}
    ...
    \end{cases}
$$
Given a point $p_n$ we calculate a proposed point $p'$. The function $d(p)$ guides our chain. If the value of $d(p')$ is higher at the new location, the chain always continues. If the value of $d(p)$ is lower at the new location the chain sometimes transitions. This guaranties that the whole space is getting sampled. This assumption only holds up if we sample for enough steps and the sampled function is compact, in addition to probably other things.
...
In a first step we have to decide the dimensions we want to uniformly sample.
```{r set-dimesnions}
...
We now can sample our environmental space uniformly. Pseudo absences should not only be uniformly distributed but also evade the conditions under which the species we try to model flourishes. We have to extend the function from above to reflect that
...
```
Now we have real points in our environment with a geographical location. The model of our environment is not perfect. The distance to the remapped point gives an indication on how far the chain wandered of the real data points. We can use it to filter points that originated in areas where our environmental model was bad. The distance that lays between a point and its origin should never be more than half the maximal distance between points in the dataset. 
...
print(p.entropy)
```
We see that the entropy stablizeses and is simmilar for our different chains, indicating that they are well mixed. 
