...
vignette: >
  %\VignetteIndexEntry{Insights on Markov chain based pseudo-absence sampling}
...
knitr::opts_chunk$set(
  ...
  ## cairo_pdf embeds glyphs/lines as vectors; the gif chunks render via magick +
...
gif.example.chain <- gganimate::animate(a.example.chain, renderer = magick_renderer(), width = 700, height = 600, fps = 10, duration = 4)
...
gif.example.chain.gaussian <- animate(a.example.chain.gaussian, renderer = magick_renderer(), width = 700, height = 600, fps = 10, duration = 6)
...
# Build one sf table per pixel: PC1/PC2 (and the bioclim layers) are ordinary
# columns, while the geographic x/y are kept as the point geometry.
...
# Cut off the lowest-density environments: keep the densest 99% of cells as the
# samplable space (raise 0.01 to exclude more of the sparse edges).
environmental.threshold <- stats::quantile(environmental.densities, 0.01)
```

We can now construct the function that our chain tries to sample from. The next
block is a *function that builds another function*: you hand `densityFunctionNaive()`
the fitted environmental model and a threshold once, and it hands back a ready-to-use
`densityFunction(point)` that scores any single point (here simply 1 inside the
environment, 0 outside). We keep that returned function in `densityFunction` and feed
it to the chain.
...
grid$density <- sapply(seq_len(nrow(grid)),               # score every grid cell
                       function(i) densityFunction(grid[i, , drop = FALSE]))
...
gif.chain.naive.density <- animate(a.chain.naive.density, renderer = magick_renderer(), width = 700, height = 400, fps = 10, duration = 10)
...
gif.chain.ps.density <- animate(a.chain.ps.density, renderer = magick_renderer(), width = 700, height = 400, fps = 10, duration = 10)
...
chain.density.grid$density <- sapply(            # score every grid cell
  seq_len(nrow(chain.density.grid)),
  function(i) densityFunction(chain.density.grid[i, , drop = FALSE]))
panel.d <- ggplot() +
...
sampled.points.mcmc.higher.dim <- paSamplingMcmc(env.rast = env.data.raster,
...
chain.list <- list()                          # run 4 independent chains
for (i in 1:4) {
  sampled.points <- mcmcSampling(dataset = env.with.pc.sf.subsampled,
                                 ...
                                 burnIn = 1000,
                                 covariance.correction = 50,  # proposal step scale; raise it as dimensions grow
                                 verbose = FALSE)
...
sampled.points.uniform.p <- paSampling(env.rast = env.data.raster,
                                       pres = virtual.presence.points, 
                                       grid.res = 10)
# paSampling now returns a single sf already in geographic coordinates.
sampled.points.uniform.p.location <- sampled.points.uniform.p
