...
#'
#' A copy of \code{stats::princomp} in which the non-negative-definite covariance
#' check issues a \code{warning()} instead of a \code{stop()} (see the L53 branch),
#' so PCA on a near-singular raster covariance still returns a result.
...
#' @param ... further arguments passed to or from other methods; currently checked via \code{chkDots()} and otherwise ignored.
#' @returns An object of class \code{"princomp"}: a list with \code{sdev}, \code{loadings}, \code{center}, \code{scale}, \code{n.obs}, \code{scores} and the matched \code{call}.
