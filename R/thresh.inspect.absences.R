...
#' @param pseudoAbsence A SpatialPointsDataframe, a SpatVector or an sf object sampled pseudo absences of the species of interest.
...
#' @export
thresh.inspect.absences <- function (env.rast, pres = NULL, pseudoAbsences = NULL, thres = 0.75, H = NULL) {

  # Check if environmental raster is valid
  if (!inherits(env.rast, "BasicRaster") && !inherits(env.rast, "SpatRaster")) {
    stop("Environmental data provided in an inconvenient form")
  }

  # Check if presence points are provided
  if (is.null(pres)) {
    stop("Species occurrences must be provided by the user")
  }

  # Ensure pres and pseudoAbsences are valid spatial objects
  ...
  # Convert data to SpatVector if it's not already
  ...
  # Check if pseudoAbsences are provided and handle them similarly to pres
  if (!is.null(pseudoAbsences)) {
    if (!inherits(pseudoAbsences, class(pres))) {
      stop("pseudoAbsences must be of the same class as pres")
    }
    # Convert pseudoAbsences to SpatVector if needed
    if (!inherits(pseudoAbsences, "SpatVector")) {
      pseudoAbs.vec <- terra::vect(pseudoAbsences)
    } else {
      pseudoAbs.vec <- pseudoAbsences
    }
  }

  cols <- c("In" = "darkorange", "Out" = "lightblue", "Presences" = "black", "PseudoAbsences" = "red")
  sizes <- c("In" = 0.5, "Out" = 0.5, "Presences" = 0.2, "PseudoAbsences" = 0.2)
  ...
  estimate <- data.frame(KDE = ks::kde(PC12ex[PC12ex$PA == 1, c("PC1", "PC2")], eval.points = PC12ex[PC12ex$PA == 1, c("PC1", "PC2")], h = H)$estimate, PC12ex[PC12ex$PA == 1, -1])
  ...
  for(i in 1:length(thres)){
    ...
    # Include pseudoAbsences in the data
    if (!is.null(pseudoAbsences)) {
      pseudoAbs.occ <- terra::extract(PC12, pseudoAbs.vec, cells = FALSE, df = TRUE)
      estimate.i <- rbind(estimate.i, data.frame(PC1 = pseudoAbs.occ$PC1,
                                                 PC2 = pseudoAbs.occ$PC2,
                                                 percP = "PseudoAbsences"))
    }
  ...
  p <- ggplot2::ggplot(estimate_list, ggplot2::aes(x = estimate_list$PC1, y = estimate_list$PC2,
    ...
    ggplot2::theme(plot.title = ggplot2::element_text(size = 16, face = 'bold'),
                   ...
                   aspect.ratio = 1,
                   legend.text = ggplot2::element_text(size = 16, angle = 0),
